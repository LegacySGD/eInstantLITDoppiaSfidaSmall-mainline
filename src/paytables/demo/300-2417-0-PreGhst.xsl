...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:x="anything">
    ...
    const qtyMatchYourSymbols = 3;

    var arrMatchWinNumIndexes = [];
    var arrWinNums = [];
    var arrYourNumParts = [];
    var arrYourNums = [];
    var arrYourSymbolCounts = [];
    var indexNum = -1;
    var indexSymbol = -1;
    var objBonusNum = {};
    var objWinNum = {};
    var objYourNum = {};
    var objYourSymbolCount = [];

    for (var indexYourNum = 0; indexYourNum < scenarioYourNums.length; indexYourNum++) {
        objYourNum = { sValue: '', sSymbol: '', bMatchWinNum: false, bMatchBonusNum: false, bMatch3Symbols: false };

        arrYourNumParts = scenarioYourNums[indexYourNum].split(':');

        objYourNum.sValue = arrYourNumParts[0];
        objYourNum.sSymbol = arrYourNumParts[1];

        arrYourNums.push(objYourNum);

        indexSymbol = arrYourSymbolCounts.map(function (item) { return item.sSymbol; }).indexOf(objYourNum.sSymbol);

        if (indexSymbol == -1) {
            objYourSymbolCount = { sSymbol: objYourNum.sSymbol, iCount: 1, arrYourSymbolIndexes: [indexYourNum] };

            arrYourSymbolCounts.push(objYourSymbolCount);
        } else {
            arrYourSymbolCounts[indexSymbol].iCount++;
            arrYourSymbolCounts[indexSymbol].arrYourSymbolIndexes.push(indexYourNum);

            if (arrYourSymbolCounts[indexSymbol].iCount == qtyMatchYourSymbols) {
                for (var indexYourSymbolIndexes = 0; indexYourSymbolIndexes < qtyMatchYourSymbols; indexYourSymbolIndexes++) {
                    arrYourNums[arrYourSymbolCounts[indexSymbol].arrYourSymbolIndexes[indexYourSymbolIndexes]].bMatch3Symbols = true;
                }
            }
        }
    }
    ...
    for (var indexWinNum = 0; indexWinNum < scenarioWinNums.length; indexWinNum++) {
        objWinNum = { sValue: '', bMatched: '' };

        arrMatchWinNumIndexes = arrYourNums.map(function (item, index) { return (item.sValue == scenarioWinNums[indexWinNum]) ? index : -1; }).filter(function (item) { return item != -1 });

        objWinNum.sValue = scenarioWinNums[indexWinNum];
        objWinNum.bMatched = (arrMatchWinNumIndexes.length != 0);

        arrWinNums.push(objWinNum);

        if (objWinNum.bMatched) {
            for (var indexMatch = 0; indexMatch < arrMatchWinNumIndexes.length; indexMatch++) {
                arrYourNums[arrMatchWinNumIndexes[indexMatch]].bMatchWinNum = true;
            }
        }
    }

    objBonusNum = { sValue: '', bMatched: '' };

    indexNum = arrYourNums.map(function (item) { return item.sValue; }).indexOf(scenarioBonusNum);

    objBonusNum.sValue = scenarioBonusNum;
    objBonusNum.bMatched = (indexNum != -1);

    if (objBonusNum.bMatched) {
        arrYourNums[indexNum].bMatchBonusNum = true;
    }
    ...
    boxColourStr = (objBonusNum.bMatched) ? colourRed : colourWhite;
    ...
    for (var indexYourNumRow = 0; indexYourNumRow < yourNumRows; indexYourNumRow++) {
        ...
        for (var indexYourNumCol = 0; indexYourNumCol < yourNumsPerRow; indexYourNumCol++) {
            ...
            boxColourStr = (arrYourNums[indexYourNum].bMatchWinNum) ? colourLime : ((arrYourNums[indexYourNum].bMatchBonusNum) ? colourRed :
                ((arrYourNums[indexYourNum].bMatch3Symbols) ? colourOrange : colourWhite));
            
            textStr1 = arrYourNums[indexYourNum].sValue;
            textStr2 = convertedPrizeValues[getPrizeNameIndex(prizeNames, arrYourNums[indexYourNum].sSymbol)];
